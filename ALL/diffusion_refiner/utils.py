...
def ddim_sample(
    ...
    for i, t_cur in enumerate(timesteps):
        ...
        # Compute alpha schedule values on the correct device
        alphas_cumprod = scheduler.alphas_cumprod.to(device)
        alphas_cumprod_prev = scheduler.alphas_cumprod_prev.to(device)
        a_t = alphas_cumprod[t].view(-1, 1, 1, 1)
        a_prev = alphas_cumprod_prev[t].view(-1, 1, 1, 1) if t[0] > 0 else torch.ones_like(a_t)
